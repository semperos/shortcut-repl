...
class ThereAndBackAgain {
  ...
  ScMap search(ScEnv env) {
    ...
    final iterationsResults =
        searchResults['iterations'] as Map<String, dynamic>;
    final milestonesResults =
        searchResults['milestones'] as Map<String, dynamic>;
    final storiesData = storyResults['data'] as List;
    final epicsData = epicResults['data'] as List;
    final iterationsData = iterationsResults['data'] as List;
    final milestonesData = milestonesResults['data'] as List;
    final stories = storiesData.map((data) => ScStory.fromMap(env, data));
    final epics = epicsData.map((data) => ScEpic.fromMap(env, data));
    final iterations =
        iterationsData.map((data) => ScIteration.fromMap(env, data));
    final milestones =
        milestonesData.map((data) => ScMilestone.fromMap(env, data));
    return ScMap({
      ScString('stories'): ScList(stories.toList()),
      ScString('epics'): ScList(epics.toList()),
      ScString('iterations'): ScList(iterations.toList()),
      ScString('milestones'): ScList(milestones.toList()),
    });
  }
}
